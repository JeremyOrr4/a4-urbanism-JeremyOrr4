...
import ca.mcmaster.cas.se2aa4.a3.pathfinder.City.CreateCity;
...
public class PathFinderEntry {
    public Structs.Mesh CreateCity(Structs.Mesh Mesh){
        ...
        List<Node> shortestDistance = Path.findPathBetweenNode(Graph, NodeList.get(2), NodeList.get(456));
        
        List<Segment> SegmentListOfPath = new ArrayList<Segment>();
        SegmentListOfPath = CityGraphCreator.GetSegmentPathList(shortestDistance, Mesh, Graph);

        return Structs.Mesh.newBuilder().addAllVertices(Mesh.getVerticesList()).addAllSegments(Mesh.getSegmentsList())
